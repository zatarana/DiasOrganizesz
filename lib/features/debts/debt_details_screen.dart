...
class _DebtDetailsScreenState extends ConsumerState<DebtDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var t in installments) {
      if (t.status == 'paid') {
         ...
         if (t.discountAmount != null) paidForThisDebt += t.discountAmount!;
      }
    }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => _confirmDelete(currentDebt),
          ),
        ],
      ),
      body: Column(
        children: [
          _buildHeader(currentDebt, paidForThisDebt, progress),
          const Divider(),
          const Padding(
  ...
  Widget _buildHeader(Debt d, double paid, double progress) {
    return Padding(
      ...
      child: Column(
         ...
         children: [
           Row(
             ...
             children: [
                Text('Valor Total:', style: TextStyle(fontSize: 16, color: Colors.grey.shade700)),
  ...
  void _toggleInstallment(FinancialTransaction t, bool isPaid) {
     final newStatus = isPaid ? 'paid' : 'pending';
